...
def create_regions(world: "ctrAPWorld"):
    ...
    # The 10-wumpa check (H-dossier ruling, 2026-08-10 16:28) is global for the
    # same reason itemsanity is -- fruit are collected wherever you race -- so it
    # hangs off Menu too, and it needs no rule of its own: reaching ten fruit is
    # possible in any race from sphere 0, so `True` is the honest access rule
    # rather than a placeholder. One location per seed, never one per track: the
    # ruling settled that directly, "by the same anti-per-track reasoning as the
