...
ACTIVATED BY `wumpa_check` (H-dossier build). The name landed inert with the
freeze -- `created_location_names` returned nothing unconditionally, because the
freeze mints names, not features. It now creates the single location when the
...
ordinary data/items.json entries at indexes 120-122, owned by `h_dossier.py`,
and are not this class's to register.
"""
...
class WumpaLocationClass(LocationClass):
    ...
    def created_location_names(self, options):
        ...
        infrastructure tests with stand-in option objects that carry only the
    ...
    def location_name_groups(self):
        """The single-name group, so `!hint Wumpa Checks` resolves the same way
        `!hint Itemsanity Checks` does."""
        return {"Wumpa Checks": {WUMPA_TEN_LOCATION}}
