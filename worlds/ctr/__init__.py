...
class ctrAPWorld(World):
    ...
    def create_items(self):
        ...
        # COMPOSITION NOTE (#49 + #50), carried over from the v1 review client.
        # This guard reads `unfilled` and `len(pool)` HERE, before the
        # progressive capability packs below are appended -- which is exactly
        # the ordering hazard #50's supply fix moved the character guard past.
        # #49's reviewed behaviour is kept as-is because the early position is
        # safe in one direction: it can only fire on a shortfall the later packs
        # would make worse, never on a phantom one. It is still two guards
        # accounting at two different points. When both halves land on `main`
        # they should be reconciled into the single authoritative accounting
        # point rather than left as they are.
