...
class ctrAPWorld(World):
    ...
    def create_items(self):
        ...
        # COMPOSITION NOTE (#49 + #50). #50 rewrote this region around the
        # comfort-pack trim, so #49's items_only shortfall guard lands here as a
        # conflicting insertion. Kept verbatim in its reviewed position. It
        # accounts BEFORE the progressive capability packs are appended, which is
        # exactly the ordering hazard #50's supply fix moved the character guard
        # past. Safe in one direction: it can only fire on a real shortfall the
        # later packs would worsen, never a phantom one. The two guards should be
        # reconciled into ONE authoritative accounting point when both halves land
        # on main. Carried unchanged from the v1 and v2 compositions; still owed.
